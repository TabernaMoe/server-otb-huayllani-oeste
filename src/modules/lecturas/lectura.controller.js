...
export class LecturaController {
  static async getAll(req, res, next) {
    try {
      ...
      const result = await services.getAll(page, limit);

      return res.status(200).json({
        ok: true,
        message: 'Soscio obtenidos correctamente',
        ...result,
      });
    } catch (e) {
      next(e);
  ...
  static async hitoryId(req, res, next) {
    try {
      const { id } = req.params;
      ...
      const data = await services.hitoryId(id);
