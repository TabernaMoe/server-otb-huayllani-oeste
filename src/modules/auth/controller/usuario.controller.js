import { UsuarioServices as services } from '../services/usuario.services.js';
...
export class UsuarioController {
  ...
  static async getId(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ ok: true, message: 'Usuario obtenido correctamente', dato });
    } catch (e) {
      next(e);
    }
  }
  ...
  static async update(req, res, next) {
    try {
      ...
      return res.status(200).json({
        ok: true,
        message: 'Usuario actuzalizado correctamente',
        dataUpdated,
      });
    } catch (e) {
      next(e);
